...
const UpdateProfile = () => {
  ...
  console.log('useParams email: ', email);
  ...
  const getData = async (email) => {
    ...
    try {
      console.log("email",email);
       ...
       console.log('response:999999999999999999 ', response.data);
  ...
  const handleInputChange = (event) => {
    ...
    if (name === 'name') {
      ...
      if (!value.match(/^\d{10}$/)) {
        validationErrors[name] = 'Invalid phone number (must be 10 digits)';
      } else {
        validationErrors[name] = '';
      }
    } else if (name === 'email') {
    ...
    <>
      <div className='d-flex flex-row container row m-3'>
        <div className="col-3">
          ...
          <div className="col-3">
            <Link to='/admin/departments'>Go Back</Link>
          </div>
